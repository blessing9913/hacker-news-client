...
function newsFeed() {
      ...
      <div class="p-6 ${newsFeed[i].read ? 'bg-red-500' : 'bg-white'} mt-6 rounded-lg shadow-md transition-colors duration-500 hover:bg-green-100">
        ...
        <div class="flex mt-3">
          <div class="grid grid-cols-3 text-sm text-gray-500">
            <div class="mr-5"><i class="fas fa-user mr-1"></i>${newsFeed[i].user}</div>
            <div class="mr-5"><i class="fas fa-heart mr-1"></i>${newsFeed[i].points}</div>
            <div class="mr-5"><i class="far fa-clock mr-1"></i>${newsFeed[i].time_ago}</div>
          </div>
        </div>
      </div>
    `);
  ...
  template = template.replace('{{__next_page__}}', newsFeed.length > store.currentPage * 10 ? store.currentPage + 1 : store.currentPage);
      ...
      if (comments[i].comments.length > 0) { // 삭제된 글은 미표시
        commentString.push(`
          <div style="padding-left: ${called * 40}px;" class="mt-4">
            ...
            <p class="text-gray-700">${regEx(comments[i].content)}</p>
          </div>
        `);
